...
float volt, ampere, avgampere; int arsample;
...
String update_data2() {
  ...
  Serial.printf("[Firebase] UPDATE with data : %s\n", send_data2.c_str());
  if (https.begin(client, lin2)) {  // HTTPS
    https.addHeader("Content-Type", "application/x-www-form-urlencoded");

    int httpCode = https.PUT(send_data2);
    if (httpCode > 0) {
      response = https.getString();
      //response=true;
      Serial.printf("[Firebase] UPDATE response code: %d\n", httpCode);
...
String update_data3() {
  ...
  Serial.printf("[Firebase] UPDATE with data : %s\n", send_data3.c_str());
  if (https.begin(client, lin3)) {  // HTTPS
    https.addHeader("Content-Type", "application/x-www-form-urlencoded");

    int httpCode = https.PUT(send_data3);
    if (httpCode > 0) {
      response = https.getString();
      //response=true;
      Serial.printf("[Firebase] UPDATE response code: %d\n", httpCode);
...
String get_data(){
  ...
  if (https.begin(client, lin1)) {  // HTTPS
...
void pump(){
  if(pompauser == "true"||TAir>ATAir){
    ...
    if(page==0){
      ...
      Serial.print("bt0.val=1");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    }
  }
  else{
    digitalWrite(2, HIGH);
    ...
    if(page==0){
      ...
      Serial.print("bt0.val=0");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    }
  }
...
void loop() {
  ...
  if(page==0&&nowmillis-beforeget>7500){pompauser=get_data();beforeget = nowmillis;}
  printWiFiStatus();
  //volarus();
  pump();
  ...
  if(nexid == "37"){
    //page2
    page=2;
    Serial.print("page 2");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    Serial.print("page 2");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  }
  else if(nexid == "16"){
    //page1
    page=1;
    Serial.print("page 1");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    Serial.print("page 1");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  }
  else if(nexid == "15"){
    //simpan
    ...
    "}");
    update_data3();
    Serial.print("t8.txt=\"Data Tersimpan!\"");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    Serial.print("t8.txt=\"Data Tersimpan!\"");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    delay(1000);
    Serial.print("t8.txt=\" \"");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  }
  else if(nexid == "20"){
    //page0
    page=0;
    Serial.print("page 0");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    Serial.print("page 0");Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  }
  else if(nexid == "60"){
    page=0;
  ...
  if(page==2){
    Serial.print("wifi.txt=\"");Serial.print(ssid);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    
    Serial.print("umur.txt=\"");Serial.print(sumur);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);

    Serial.print("harga.txt=\"");Serial.print(sharga);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);

    Serial.print("luas.txt=\"");Serial.print(sluas);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);

    Serial.print("pupuk.txt=\"");Serial.print(spupuk);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);

    if(con==0){input();}
    else if(con==1){keypadumur();}
    else if(con==2){keypadharga();}
    else if(con==3){keypadluas();}
    else if(con==4){keypadpupuk();}
  }
...
void TempTinggi(){
  ...
  Serial.print("suhu.txt=\"");Serial.print(temp);Serial.print("°C\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  float RTAir = distanceSensor.measureDistanceCm(temp);//TAir[0]
  TAir = 11.5 - RTAir;
  int airValue = TAir/8*100;
  Serial.print("air.txt=\"");Serial.print(TAir);Serial.print(" cm\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  Serial.print("airValue.val=");Serial.print(airValue);
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  if(temp < 18){
...
void LembapTanah(){
  ...
  Serial.print("kelembapan.txt=\"");Serial.print(kelembapan);Serial.print("%\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  if(kelembapan < 65){
...
void volarus(){
  ...
  if(page==1){
    Serial.print("Tegangan.txt\"");Serial.print(volt);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
    Serial.print("Arus.txt\"");Serial.print(ampere);Serial.print("\"");
    Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  }
  delay(50);
}
...
*/
void Fuzzy(){
  if(temp<18 && kelembapan<65){//dingin kering
    fuzzy = "gagal";
    kali = 0;
    predict = 0;
  ...
  else if(temp<18 && 65<=kelembapan<93){//dingin lembap
    fuzzy = "sedang";
    kali = 0.7;
    predict = 2;
  ...
  else if(temp<18 && 93<=kelembapan){//dingin basah
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
  ...
  else if(18<=temp<25 && kelembapan<65){//sejuk kering
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
    Serial.print("t12.txt=\"Kurang\"");
  ...
  else if(18<=temp<25 && 65<=kelembapan<93){//sejuk lembap
    fuzzy = "melimpah";
    kali = 1.246;
    predict = 4;
  ...
  else if(18<=temp<25 && 93<=kelembapan){//sejuk basah
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
    Serial.print("t12.txt=\"Kurang\"");
  ...
  else if(25<=temp<33 && kelembapan<65){//hangat kering
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
    Serial.print("t12.txt=\"Kurang\"");
  ...
  else if(25<=temp<33 && 65<=kelembapan<93){//hangat lembap
    fuzzy = "berhasil";
    kali = 1;
    predict = 3;
  ...
  else if(25<=temp<33 && 93<=kelembapan){//hangat basah
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
    Serial.print("t12.txt=\"Kurang\"");
  ...
  else if(33<=temp && kelembapan<65){//panas kering
    fuzzy = "gagal";
    kali = 0;
    predict = 0;
    Serial.print("t12.txt=\"Gagal\"");
  ...
  else if(33<=temp && 65<=kelembapan<93){//panas lembap
    fuzzy = "sedang";
    kali = 0.7;
    predict = 2;
    Serial.print("t12.txt=\"Sedang\"");
  ...
  else if(33<=temp && 93<=kelembapan){//panas basah
    fuzzy = "kurang";
    kali = 0.4;
    predict = 1;
    Serial.print("t12.txt=\"Kurang\"");
...
void prediksi(){
  ...
  hasil = luas * kali;
  total = hasil * harga;
  Serial.print("t13.txt=\"");Serial.print(hasil);Serial.print("Kg\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  Serial.print("t14.txt=\"Rp");Serial.print(harga);Serial.print("\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  Serial.print("t19.txt=\"Rp");Serial.print(total);Serial.print("\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
  Serial.print("LTanah.txt=\"");Serial.print(luas);Serial.print(" m2\"");
  Serial.write(0xff);Serial.write(0xff);Serial.write(0xff);
}
